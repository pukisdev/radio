...
app.controller('dataFormController', function($scope, $http) {
  ...
  $scope.subUrl = '/prod/realisasi';
  $scope.tayang_realisasi = {};
...
*/
    $scope.save = function(value, id){
      ...
      _temp = value;
      angular.forEach($scope._data, function(item, key){
        _temp[key]['pnwr'] = item.f_pnwr;
      });
...
*/
  function getResultsPage(_id) {
     var _url = $scope._token['url']+ $scope.subUrl;
     console.log(_url);
     // var _url = $scope._token['url']+ $scope.subUrl+"/"+id+'?_token='+$scope._token['token']+'&_e='+$scope._token['e'];
      if(! $.isEmptyObject($scope._token['url'])){
          $http({
            'method'  : 'GET',
           'url'     : _url+((_id.length > 0) ? _id : '/NONID')
           // 'url'     : $scope.subUrl+'/NONID/?page='+pageNumber 
          }).success(function(response){
            // console.log(response.data);
            $scope._tanggal  = new Date(response.tanggal.date);
            $scope._data    = response.data;
            $scope._jam     = Array.apply(0, Array(16)).map(function(val, i) { return i; }); //ini untuk mendefine pajang kolom jam yang ingin dibentuk
