...
app.controller('dataFormController', function($scope, $http, $uibModal, $log){//, API_URL) {
  ...
  $scope._pageNumber=1;
        ...
        case 'edit' :
          ...
          $http({
            ...
            'headers' : {'Content-Type': 'application/x-www-form-urlencoded'}
              ...
              if(! $.isEmptyObject(response.spks)) $scope.dataForm['alias_spks']   =  response.spks['alias_spks'];
          });
          $scope.formTampil = true;
        break;
        case 'cancel':
          // console.log('mana ?'+$scope._pageNumber);
          $scope.dataForm = {};
          getResultsPage($scope._pageNumber);
    ...
    $scope.save = function(modalstate, id){
      ...
      // console.log(id);
      // console.log($.param($scope.dataForm));
      var _method = (modalstate === 'edit') ? 'PUT' : 'POST';
      $http({
        'method'  : _method,
        'url'     : _url + ((modalstate ==='edit') ? '/edit' : ''),
    ...
    $scope.getFilesSpks = function(_name){
...
*/
  ...
  $scope.lovAe = function (_index, _tipe, _value) {
      if($.isEmptyObject($scope.dataForm)){
        $scope.dataForm = {};
      }
      // console.log('mane?');
    ...
    modalInstance.result.then(function (_element) {
      ...
      $scope.dataForm['f_ae'] = _element.nama;
    }, function () {
  ...
  $scope.lovProdukSelected = function(idModal, _element){
    ...
    $scope.dataForm['durasi'] = parseInt(_element.durasi); 
  ...
  $scope.openTayang = function () {
    ...
    $scope.params = { _tokenUrl : _url, _periode : $scope.dataForm['periode'] , _frekuensi : $scope.dataForm['frekuensi'], _f_pnwr : $scope.dataForm['id_pnwr'] };
    // console.log($scope.dataForm['periode']);
    // console.log($scope.dataForm);
    // console.log($scope.params['periode']);
    var modalInstance = $uibModal.open({
      animation: true,
      templateUrl: '/assets/ng/views/pms/forms/pnwrTayang.html',
  ...
  $scope.openMateri = function () {
    ...
    $scope.params = { _tokenUrl : _url, _f_pnwr : $scope.dataForm['id_pnwr']  };
    var modalInstance = $uibModal.open({
      animation: true,
      templateUrl: '/assets/ng/views/pms/forms/pnwrMateri.html',
  ...
  $scope.lovSpks = function (_customer, _index) {
      if($.isEmptyObject($scope.dataForm)){
        $scope.dataForm = {};
      }
    ...
    var modalInstance = $uibModal.open({
      animation: true,
      templateUrl:  '/assets/ng/views/hkm/lov/lovSpks.html', 
      controller: 'lovSpksController',
      size: 'lg',
      resolve: {
        parameter : function(){
          return $scope.params;
    ...
    modalInstance.result.then(function (_element) {
      ...
      if(_index || _index ==0){
        $scope.dataForm[_index]= {} ;
        $scope.dataForm[_index]['f_spks']     = _element.id_spks;
        $scope.dataForm[_index]['alias_spks'] = _element.alias_spks;
      } else {
        $scope.dataForm['f_spks'] = _element.id_spks;
        $scope.dataForm['alias_spks'] = _element.alias_spks;
      } 
    }, function () {
      $log.info('Modal dismissed at: ' + new Date());
    });
  ...
  $scope.simpanSpks = function(_id, _index){
      var _url = API_URL + $scope.subUrl;
      $scope.dataForm[_index]['id_pnwr'] = _id;
      // console.log(id);
      $http({
        'method'  : 'PUT',
        'url'     : _url+'/spks/rianday',
        'data'    : $.param($scope.dataForm[_index]),
        'headers' : {'Content-Type': 'application/x-www-form-urlencoded'}
      }).success(function(response){
...
app.directive('formPnwrMst', function (){
    return{
        restirct    : 'A',
        templateUrl : '/assets/ng/views/pms/forms/formPnwrMst.html',
    };
});
...
app.directive('lovCustomer', function (){
    return{
        restirct    : 'A',
        templateUrl : '/assets/ng/views/pms/lov/lovCustomer.html',
    };
});
...
app.directive('lovProduk', function (){
    return{
        restirct    : 'A',
        templateUrl : '/assets/ng/views/pms/lov/lovProduk.html',
    };
});

// app.directive('lovSpks', function (){
//     return{
//         restirct    : 'A',
//         templateUrl : '/ext/ng-html/hkm/lov/lovSpks.html',
//     };
// });
...
app.controller('ModalInstanceCtrl', function ($scope, $http, $uibModalInstance, parameter){//, API_URL) {
  ...
  // console.log($scope.params);
  // alert('kenape nih?');
  $scope._periode = Array.apply(0, Array(parseInt($scope.parameter['_periode']))).map(function(val, i) { return i; });
  $scope._frekuensi = Array.apply(0, Array(parseInt($scope.parameter['_frekuensi']))).map(function(val, i) { return i; });
  ...
  $scope.simpan = function (modalstate, id) {
    ...
    _url = $scope.parameter['_tokenUrl'] + $scope.subUrl;
