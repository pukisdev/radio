app.controller('lovCustomerController', function($scope, $http, $uibModalInstance, parameter){//, API_URL) {
  ...
  $scope.lovCustomerSelected = function(_element){
  ...
  function getResultsPageCustomer(pageNumber) {
      ...
      if(! $.isEmptyObject($scope.libraryTempCustomer)){
        $http({
          ...
          'url'     : _url+'?search='+$scope.searchTextCustomer+'&page='+pageNumber+'&coa_id='+$scope.parameter['coa_id'] 
        }).success(function(response){
          // console.log(response);
        ...
        $http({
          ...
          'url'   : _url+'?page='+pageNumber+'&coa_id='+$scope.parameter['coa_id'] 
        }).success(function(response){
